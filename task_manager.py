...
def main() -> None:
    ...
    while True:
        ...
        username: str = input("Please provide your valid username: \n")
        password: str = input(f"{username}, Please provide your valid password: \n")
    ...
    : ''').lower()
        ...
        menu_chars = ['r', 'a', 'va', 'vm', 'st', 'e']

        if menu in menu_chars:

            # Registering a new user
            if menu == 'r':
                if username == 'admin':
                    # Requesting user for inputs
                    new_username: str = input("Please provide a username: \n")
                    password: str = input("Please provide a password: \n")
                    password_confirmation: str = input(f"Please {new_username}, confirm your password. \n")
                    ...
                    if password == password_confirmation:
                        ...
                        print(
                            f"{new_username},your password is not matching your confirmed password, \n restart and try again")
                        continue
            ...
            # Adding a task for a user, this option can only be performed by the admin.
                ...
                with open(user_txt, 'r') as logged_admin:
                    # Requesting user for inputs
                    user: str = input("Please a provide username for the person to be tasked: \n")
                    title_of_task: str = input(f"Please enter the title task for {user}:  \n")
                    description_of_task: str = input(f"Now enter a description for the task: \n")
                    due_date: str = input("Please provide the due date for the task in this format (DD MM YY): \n")
                    task_completed = "No"
            ...
            # Viewing statistics
                ...
                with open(task_txt, 'r') as admin_logged:
                    ...
                    total_num_of_tasks: int = len(space_removed)
